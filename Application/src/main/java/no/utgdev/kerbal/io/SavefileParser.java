...
public class SavefileParser {
    ...
    public Savefile parse() {
        try {
            ...
            logger.debug("\t{}", Arrays.toString(currentPropertyMap.toArray()));
        } catch (Exception e) {
            logger.error("An error accured while parsing " + name + ". Returning empty savefile.", e);
            return new Savefile(I18N.getInstance().getString("nofile"));
        }
        return savefile;
    }

    private void process(ParserEvents event, String... strings) throws IllegalArgumentException {
        try {
            switch (event) {
                case TAG:
                ...
                case KEYVALUE:
                    ...
                    break;
                default:
                    throw new IllegalArgumentException("Invalid parserEvent: " + event);
            }
        } catch (Exception e) {
            throw new IllegalArgumentException("An error accured while processing a ParseEvent: " + event, e);
        }
    }
}
